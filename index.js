...
firebase.auth().onAuthStateChanged(async function(user) {
  ...
  if (user) {
    ...
    for (let i=0; i < json.length; i++) {
      ...
      console.log(`number of likes in the database is ${post.numberOfLikes}`)
      ...
      commentButton.addEventListener(`click`, async function(event) {
        ...
        let commentUrlInput = document.querySelector(`#comment-${postId}`)

        // get the body of the comment
        let commentInput = commentUrlInput.value

        // Build the URL for our posts API
        let url = `/.netlify/functions/create_comment?postId=${postId}&userName=${user.displayName}&body=${commentInput}`
      ...
      likeButton.addEventListener(`click`, async function(event) {
        ...
        let url = `/.netlify/functions/create_like?uid=${user.uid}&postId=${postId}`
